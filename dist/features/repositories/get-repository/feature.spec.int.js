...
describeOrSkip('getRepository integration', () => {
    ...
    test('should retrieve a real repository from Azure DevOps', async () => {
        ...
        const result = await (0, feature_1.getRepository)(connection, projectName, testRepo.name || testRepo.id || '');
    ...
    test('should throw error when repository is not found', async () => {
        ...
        await expect((0, feature_1.getRepository)(connection, projectName, nonExistentRepoName)).rejects.toThrow(/not found|Failed to get repository/);
    });
});
