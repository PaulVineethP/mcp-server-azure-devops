...
const rethrowCodeSearchError = (error) => {
...
describeOrSkip('searchCode integration', () => {
    ...
    test('should search code in a project', async () => {
        ...
        try {
            ...
            const result = await (0, feature_1.searchCode)(connection, options);
    ...
    test('should filter results when filters are provided', async () => {
        try {
            ...
            const initialResult = await (0, feature_1.searchCode)(connection, initialOptions);
            ...
            const result = await (0, feature_1.searchCode)(connection, filteredOptions);
            // Assert on the actual response
            expect(result).toBeDefined();
            // All results should be from the specified repository
            if (result.results.length > 0) {
                const allFromRepo = result.results.every((r) => r.repository.name === repoName);
    ...
    test('should handle pagination', async () => {
        try {
            ...
            const firstPageResult = await (0, feature_1.searchCode)(connection, firstPageOptions);
            ...
            const secondPageResult = await (0, feature_1.searchCode)(connection, secondPageOptions);
            ...
            if (firstPageResult.results.length > 0 &&
                ...
                const hasOverlap = firstPagePaths.some((path) => secondPagePaths.includes(path));
