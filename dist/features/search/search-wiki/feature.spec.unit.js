...
describe('searchWiki unit', () => {
    ...
    test('should return wiki search results with project ID', async () => {
        ...
        // Act
        const result = await (0, feature_1.searchWiki)(mockConnection, {
            searchText: 'example',
        ...
        expect(mockedAxios.post).toHaveBeenCalledWith(expect.stringContaining('https://almsearch.dev.azure.com/testorg/TestProject/_apis/search/wikisearchresults'), expect.objectContaining({
            searchText: 'example',
    ...
    test('should perform organization-wide wiki search when projectId is not provided', async () => {
        ...
        expect(mockedAxios.post).toHaveBeenCalledTimes(1);
        expect(mockedAxios.post).toHaveBeenCalledWith(expect.stringContaining('https://almsearch.dev.azure.com/testorg/_apis/search/wikisearchresults'), expect.not.objectContaining({
            filters: expect.objectContaining({
    ...
    test('should build server search URL when using Azure DevOps Server', async () => {
        ...
        await (0, feature_1.searchWiki)(serverConnection, {
            searchText: 'example',
            projectId: 'ProjectX',
        });
        expect(mockedAxios.post).toHaveBeenCalledWith('https://ado.local/tfs/DefaultCollection/ProjectX/_apis/search/wikisearchresults?api-version=7.1', expect.any(Object), expect.any(Object));
    });
    test('should require projectId for server wiki search', async () => {
        ...
        await expect((0, feature_1.searchWiki)(serverConnection, {
            searchText: 'example',
