...
const MockedWebApi = azure_devops_node_api_1.WebApi;
describe('searchWorkItems', () => {
    ...
    beforeEach(() => {
        ...
        connection = new MockedWebApi('https://dev.azure.com/mock-org', mockAuthHandler);
        connection.serverUrl = 'https://dev.azure.com/mock-org';
        connection.getCoreApi.mockResolvedValue({
    ...
    it('should search work items with the correct parameters', async () => {
        // Act
        const result = await (0, feature_1.searchWorkItems)(connection, options);
        // Assert
        expect(mockedAxios.post).toHaveBeenCalledWith('https://almsearch.dev.azure.com/mock-org/mock-project/_apis/search/workitemsearchresults?api-version=7.1', {
            searchText: 'test query',
            $skip: 0,
            $top: 50,
            filters: {
    ...
    it('should include filters when provided', async () => {
        ...
        // Act
        await (0, feature_1.searchWorkItems)(connection, options);
        // Assert
    ...
    it('should handle 404 errors correctly', async () => {
        ...
        mockedAxios.post.mockImplementation(() => {
            throw new errors_1.AzureDevOpsResourceNotFoundError('Resource not found: Project not found');
        });
        // Act & Assert
        await expect((0, feature_1.searchWorkItems)(connection, options)).rejects.toThrow(errors_1.AzureDevOpsResourceNotFoundError);
    });
    it('should handle 400 errors correctly', async () => {
        // Arrange - Mock the implementation to throw the specific error
        mockedAxios.post.mockImplementation(() => {
            throw new errors_1.AzureDevOpsValidationError('Invalid request: Invalid query');
        });
        // Act & Assert
        await expect((0, feature_1.searchWorkItems)(connection, options)).rejects.toThrow(errors_1.AzureDevOpsValidationError);
    });
    it('should handle 401/403 errors correctly', async () => {
        // Arrange - Mock the implementation to throw the specific error
        mockedAxios.post.mockImplementation(() => {
            throw new errors_1.AzureDevOpsPermissionError('Permission denied: Permission denied');
        });
        // Act & Assert
        await expect((0, feature_1.searchWorkItems)(connection, options)).rejects.toThrow(errors_1.AzureDevOpsPermissionError);
    });
    it('should handle other axios errors correctly', async () => {
        // Arrange - Mock the implementation to throw the specific error
        mockedAxios.post.mockImplementation(() => {
            throw new errors_1.AzureDevOpsError('Azure DevOps API error: Internal server error');
        });
        // Act & Assert
        await expect((0, feature_1.searchWorkItems)(connection, options)).rejects.toThrow(errors_1.AzureDevOpsError);
    });
    ...
    it('should throw an error if organization cannot be extracted', async () => {
        // Arrange
        connection.serverUrl = 'https://invalid-url';
        // Act & Assert
        await expect((0, feature_1.searchWorkItems)(connection, options)).rejects.toThrow(errors_1.AzureDevOpsValidationError);
    });
    it('should use Azure Identity authentication when AZURE_DEVOPS_AUTH_METHOD is azure-identity', async () => {
        ...
        axios_1.default.post.mockResolvedValueOnce(mockResponse);
        // Call the function
        await (0, feature_1.searchWorkItems)(mockConnection, {
            projectId: 'testproject',
            searchText: 'test query',
        });
        // Verify the axios post was called with a Bearer token
        expect(axios_1.default.post).toHaveBeenCalledWith(expect.any(String), expect.any(Object), {
            headers: {
    ...
    test('should perform organization-wide work item search when projectId is not provided', async () => {
        ...
        // Act
        const result = await (0, feature_1.searchWorkItems)(connection, {
            searchText: 'bug',
        });
        // Assert
        ...
        expect(mockedAxios.post).toHaveBeenCalledWith(expect.stringContaining('https://almsearch.dev.azure.com/mock-org/_apis/search/workitemsearchresults'), expect.not.objectContaining({
    ...
    it('should use organizationId override for services search', async () => {
        ...
        await (0, feature_1.searchWorkItems)(connection, {
            searchText: 'override',
            projectId: 'mock-project',
            organizationId: 'override-org',
        });
        expect(mockedAxios.post).toHaveBeenCalledWith('https://almsearch.dev.azure.com/override-org/mock-project/_apis/search/workitemsearchresults?api-version=7.1', expect.any(Object), expect.any(Object));
    });
    it('should build server search URL when using Azure DevOps Server', async () => {
        ...
        await (0, feature_1.searchWorkItems)(serverConnection, {
            searchText: 'server',
            projectId: 'ProjectX',
        });
        expect(mockedAxios.post).toHaveBeenCalledWith('https://ado.local/tfs/DefaultCollection/ProjectX/_apis/search/workitemsearchresults?api-version=7.1', expect.any(Object), expect.any(Object));
    });
    it('should require projectId for server work item search', async () => {
        ...
        await expect((0, feature_1.searchWorkItems)(serverConnection, {
            searchText: 'server',
