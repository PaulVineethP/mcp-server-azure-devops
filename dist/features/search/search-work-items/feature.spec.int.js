...
describeOrSkip('searchWorkItems (Integration)', () => {
    ...
    it('should support pagination', async () => {
        // Act - Get first page
        const firstPage = await (0, feature_1.searchWorkItems)(connection, {
            searchText: 'test',
            projectId,
        ...
        if (firstPage.count > 5) {
            // Act - Get second page
            const secondPage = await (0, feature_1.searchWorkItems)(connection, {
                searchText: 'test',
                projectId,
                orderBy: [{ field: 'System.CreatedDate', sortOrder: 'DESC' }],
                top: 5,
            ...
            if (firstPage.results.length > 0 && secondPage.results.length > 0) {
                const firstPageIds = firstPage.results.map((r) => r.fields['system.id']);
                const secondPageIds = secondPage.results.map((r) => r.fields['system.id']);
                ...
                const hasNewItem = secondPageIds.some((id) => !firstPageIds.includes(id));
    ...
    it('should support sorting', async () => {
        ...
        if (result.results.length > 1) {
            const dates = result.results
                ...
                .map((r) => new Date(r.fields['system.createddate']).getTime());
    ...
    if (process.env.AZURE_DEVOPS_AUTH_METHOD?.toLowerCase() === 'azure-identity') {
        const azureIdentityOrgUrl = process.env.AZURE_DEVOPS_ORG_URL || '';
        const azureIdentityProjectId = process.env.TEST_PROJECT_ID || '';
        const azureIdentityTest = azureIdentityOrgUrl && azureIdentityProjectId ? test : test.skip;
        azureIdentityTest('should search work items using Azure Identity authentication', async () => {
            // Create a config with Azure Identity authentication
            const testConfig = {
                organizationUrl: azureIdentityOrgUrl,
                authMethod: auth_1.AuthenticationMethod.AzureIdentity,
                defaultProject: azureIdentityProjectId,
            };
            // Create the connection using the config
            const connection = await (0, server_1.getConnection)(testConfig);
            // Search work items
            const result = await (0, feature_1.searchWorkItems)(connection, {
