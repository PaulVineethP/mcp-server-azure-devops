...
const feature_2 = require("../add-pull-request-comment/feature");
const feature_3 = require("../create-pull-request/feature");
...
describeOrSkip('getPullRequestComments integration', () => {
    ...
    beforeAll(async () => {
            ...
            commits: [
                {
                    ...
                    changes: [
                        {
                            changeType: GitInterfaces_1.VersionControlChangeType.Add,
        ...
        const pr = await (0, feature_3.createPullRequest)(connection, projectName, repositoryId, {
        ...
        const thread = await (0, feature_2.addPullRequestComment)(connection, projectName, repositoryId, pullRequestId, {
    ...
    test('should get all comment threads from pull request with file path and line number', async () => {
        const threads = await (0, feature_1.getPullRequestComments)(connection, projectName, repositoryId, pullRequestId, {
            projectId: projectName,
            repositoryId,
            pullRequestId,
        });
    ...
    test('should get a specific comment thread by ID with file path and line number', async () => {
        ...
        expect(comment.content).toBe(`Test comment thread ${timestamp}-${randomSuffix}`);
    ...
    test('should handle pagination with top parameter', async () => {
        const allThreads = await (0, feature_1.getPullRequestComments)(connection, projectName, repositoryId, pullRequestId, {
            projectId: projectName,
            repositoryId,
            pullRequestId,
        });
        const paginatedThreads = await (0, feature_1.getPullRequestComments)(connection, projectName, repositoryId, pullRequestId, {
            projectId: projectName,
            repositoryId,
            pullRequestId,
            top: 1,
