...
describeOrSkip('listWorkItems integration', () => {
    ...
    beforeAll(async () => {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            try {
                const workItem = await (0, feature_2.createWorkItem)(connection, projectName, 'Task', options);
    ...
    test('should list work items from a project', async () => {
        ...
        const result = await (0, feature_1.listWorkItems)(connection, options);
    ...
    test('should apply pagination options', async () => {
        ...
        const allItems = await (0, feature_1.listWorkItems)(connection, allOptions);
        // Then get with pagination
        const paginationOptions = {
            projectId: projectName,
            top: 2, // Only get first 2 items
        };
        const paginatedResult = await (0, feature_1.listWorkItems)(connection, paginationOptions);
    ...
    test('should list work items with custom WIQL query', async () => {
        ...
        const foundCreatedItem = result.some((item) => createdWorkItemIds.includes(item.id || -1));
