...
jest.mock('../../../shared/errors/handle-request-error', () => {
    const actual = jest.requireActual('../../../shared/errors/handle-request-error');
    return {
        ...actual,
        handleRequestError: jest.fn((error, context) => actual.handleRequestError(error, context)),
    };
});
describe('createWikiPage Feature', () => {
    let client;
    const mockPut = jest.fn();
    const mockHandleRequestError = handle_request_error_1.handleRequestError;
    const defaultParams = {
        wikiId: 'test-wiki',
        content: 'Hello world',
        pagePath: '/',
    ...
    beforeEach(() => {
        ...
        const actual = jest.requireActual('../../../shared/errors/handle-request-error');
        mockHandleRequestError.mockImplementation((error, context) => actual.handleRequestError(error, context));
        client = {
            put: mockPut,
            defaults: {
    ...
    it('should call client.put with correct URL and data for default org and project', async () => {
        ...
        await (0, feature_1.createWikiPage)(defaultParams, client);
        expect(mockPut).toHaveBeenCalledTimes(1);
        expect(mockPut).toHaveBeenCalledWith('defaultOrg/defaultProject/_apis/wiki/wikis/test-wiki/pages?path=%2F&api-version=7.1-preview.1', { content: 'Hello world' });
    });
    it('should call client.put with correct URL when projectId is explicitly provided', async () => {
        ...
        await (0, feature_1.createWikiPage)(paramsWithProject, client);
        expect(mockPut).toHaveBeenCalledWith('defaultOrg/customProject/_apis/wiki/wikis/test-wiki/pages?path=%2F&api-version=7.1-preview.1', { content: 'Hello world' });
    });
    it('should call client.put with correct URL when organizationId is explicitly provided', async () => {
        ...
        await (0, feature_1.createWikiPage)(paramsWithOrg, client);
        expect(mockPut).toHaveBeenCalledWith('customOrg/defaultProject/_apis/wiki/wikis/test-wiki/pages?path=%2F&api-version=7.1-preview.1', { content: 'Hello world' });
    });
    it('should call client.put with correct URL when projectId is null (project-level wiki)', async () => {
        ...
        await (0, feature_1.createWikiPage)(paramsWithNullProject, clientWithoutProject);
        expect(mockPut).toHaveBeenCalledWith('defaultOrg/_apis/wiki/wikis/test-wiki/pages?path=%2F&api-version=7.1-preview.1', { content: 'Hello world' });
    });
    it('should correctly encode pagePath in the URL', async () => {
        ...
        await (0, feature_1.createWikiPage)(paramsWithPath, client);
        expect(mockPut).toHaveBeenCalledWith('defaultOrg/defaultProject/_apis/wiki/wikis/test-wiki/pages?path=%2FMy%20Test%20Page%2FSub%20Page&api-version=7.1-preview.1', { content: 'Hello world' });
    });
    ...
    it('should include comment in request body when provided', async () => {
        ...
        await (0, feature_1.createWikiPage)(paramsWithComment, client);
        expect(mockPut).toHaveBeenCalledWith('defaultOrg/defaultProject/_apis/wiki/wikis/test-wiki/pages?path=%2F&api-version=7.1-preview.1', { content: 'Hello world', comment: 'Initial page creation' });
    });
    it('should return the data from the response on success', async () => {
        ...
        const result = await (0, feature_1.createWikiPage)(defaultParams, client);
    ...
    it('should throw if organizationId is not provided and not set in defaults', async () => {
        ...
        await expect((0, feature_1.createWikiPage)(paramsNoOrg, clientWithoutOrg)).rejects.toThrow('Organization ID is not defined');
    ...
    it('should call handleRequestError if client.put throws an error', async () => {
        ...
        await expect((0, feature_1.createWikiPage)(defaultParams, client)).rejects.toThrow('Handled Error');
        expect(mockHandleRequestError).toHaveBeenCalledTimes(1);
        expect(mockHandleRequestError).toHaveBeenCalledWith(error, 'Failed to create or update wiki page');
    });
});
