...
describe('addPullRequestComment', () => {
    ...
    test('should create a new thread on a file with line number', async () => {
        ...
        const mockThread = {
            id: 789,
            status: GitInterfaces_1.CommentThreadStatus.Active, // Add missing status
            comments: [mockComment],
            threadContext: {
                filePath: '/src/app.ts',
                rightFileStart: {
    ...
    test('should derive repositoryId from pullRequestId when repositoryId is omitted', async () => {
        ...
        // Act
        const result = await (0, feature_1.addPullRequestComment)(mockConnection, 'test-project', undefined, 123, {
        ...
        expect(mockGitApi.getPullRequestById).toHaveBeenCalledWith(123, 'test-project');
        expect(mockGitApi.createThread).toHaveBeenCalledWith(expect.any(Object), 'derived-repo', 123, 'test-project');
    });
});
