...
exports.searchWorkItems = searchWorkItems;
 ...
 */
async function searchWorkItems(connection, options) {
    try {
        ...
        if (baseUrls.type === 'server' && !options.projectId) {
            throw new errors_1.AzureDevOpsValidationError('Project ID is required for Azure DevOps Server work item search');
        }
        ...
        throw new errors_1.AzureDevOpsError(`Failed to search work items: ${error instanceof Error ? error.message : String(error)}`);
    }
}
/**
 * Get the authorization header from the connection
